    ...
    def checkpoint_policy(self, copied_policy: Policy) -> None:
        ...
        if all(p is None for p in self.policy_assignments[: 2 * self.num_old_policies]):
            for i in range(self.num_old_policies):
                # Learner is always player 0, old policy is player 1 (index 2i+1).
                # For alternating player (bot_always_player_2=False), old policy
                # fills the odd-indexed slot (player 1) in each env pair.
                # For bot_always_player_2 mode, always index 2i+1.
                self.policy_assignments[2 * i + 1] = copied_policy
