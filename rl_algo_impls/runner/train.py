...
def train(args: TrainArgs):
    ...
    if config.device_hyperparams.get("torch_compile", False):
        compile_mode = config.device_hyperparams.get("compile_mode", "reduce-overhead")
        policy = compile_policy(policy, mode=compile_mode)
        # Update policy reference in algo so forward/value/distribution
        # calls go through the compiled graph
        algo.policy = policy
