...
def compile_policy(policy: "Policy", mode: str = "reduce-overhead") -> "Policy":
    """Apply torch.compile to policy for JIT acceleration (1.2-2x speedup).

    Compiles the entire policy module so that forward(), value(),
    distribution_and_value(), and step() are all JIT-compiled.

    Uses mode="reduce-overhead" by default which balances compilation time
    with runtime speed. Use mode="default" or mode="max-autotune" for
    potentially better speed at the cost of longer compilation.
    """
    import torch
    if not hasattr(torch, "compile"):
        logging.warning("torch.compile not available (requires PyTorch >= 2.x)")
        return policy
    logging.info(f"Applying torch.compile to policy with mode={mode}")
    return torch.compile(policy, mode=mode)
