...
from rl_algo_impls.shared.autocast import maybe_autocast, safe_amp_forward
...
class PPO(Algorithm):
    def __init__(
        ...
        autocast_amp_dtype: str = "bfloat16",
        ...
        clip_range_schedule_min: float = 0.1,
    ) -> None:
        super().__init__(
            policy,
            device,
        ...
        self.clip_range_schedule = clip_range_schedule
        self.clip_range_schedule_min = clip_range_schedule_min
        self._initial_clip_range = clip_range
        ...
        self.autocast_amp_dtype = autocast_amp_dtype
    ...
    def learn_epoch(
        ...
        pi_clip = self._compute_clip_range(timesteps_elapsed)
        chart_scalars = {
        ...
        for _ in range(self.n_epochs):
            ...
            for mb in r.minibatches(
                ...
                with torch.no_grad():
                    ...
                    current_entropy = entropy.mean().cpu().numpy().item()
                if self.kl_cutoff is not None and approx_kl > self.kl_cutoff:
                    pi_coef = 0

                if self.adaptive_entropy and self.target_entropy is not None:
                    ent_error = self.target_entropy - current_entropy
                    self.ent_coef = max(
                        0.001,
                        self.ent_coef * np.exp(0.1 * ent_error),
                    )
    ...
    def _compute_clip_range(self, timesteps_elapsed: int) -> float:
        """Compute clip range with optional scheduling.

        When clip_range_schedule is set, gradually increases clip_range from
        clip_range_schedule_min to the full clip_range over training, following
        the schedule type (cosine, linear, or exponential).
        """
        if not self.clip_range_schedule:
            return self.clip_range

        min_val = self.clip_range_schedule_min
        max_val = self._initial_clip_range

        total = getattr(self, "_train_timesteps", timesteps_elapsed + 1)
        progress = min(timesteps_elapsed / max(total, 1), 1.0)

        schedule = self.clip_range_schedule
        if schedule == "cosine":
            factor = 1 - (1 + np.cos(np.pi * progress)) / 2
        elif schedule == "linear":
            factor = progress
        elif schedule == "exponential":
            # Start fast, converge slowly: 1 - exp(-5 * progress)
            factor = 1 - np.exp(-5 * progress)
        else:
            return self.clip_range
